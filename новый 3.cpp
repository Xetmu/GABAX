...
std::unordered_map<std::string, std::shared_ptr<Client>> clients_2;
        ...
        try {
            ...
            if (type == "register") {
                ...
                std::string room = json::value_to<std::string>(val.at("room"));
                {
                    ...
                    if (clients_2.count(client_name)) {
                        if (!clients[client_name]->room.empty()) {
                            rooms[clients_2[client_name]->room].erase(client_name);
                        }
						std::lock_guard<std::mutex> lock(global_mutex);
                        clients_2[client_name]->room = room;						
